...
public sealed class DslCommand : DbCommand, IDslPreparedCommand, IDescriptorFiller, ICloneable
{
	static readonly IFbLogger Log = FbLogManager.CreateLogger(nameof(DslCommand));
	...
	private DslTransaction _transaction;
	private DslParameterCollection _parameters;
	private StatementBase _statement;
	private DslDataReader _activeReader;
	...
	public override int CommandTimeout
	{
		...
		set
		{
			if (value < 0)
			{
				Diag.Dug(true, "The property value assigned is less than 0.");
	...
	public new DslConnection Connection
	{
		...
		set
		{
			...
			{
				Diag.Dug(true, "There is already an open DataReader associated with this Command which must be closed first.");
	...
	public new DslParameterCollection Parameters
	{
		get
		{
			_parameters ??= new DslParameterCollection();
			return _parameters;
		}
	}
	...
	public new DslTransaction Transaction
	{
	...
	protected override DbConnection DbConnection
	{
		...
		set { Connection = (DslConnection)value; }
	}

	protected override DbTransaction DbTransaction
	{
		get { return Transaction; }
		set { Transaction = (DslTransaction)value; }
	}
	...
	internal DslDataReader ActiveReader
	{
		get { return _activeReader; }
		set { _activeReader = value; }
	}

	internal DslTransaction ActiveTransaction
	{
	...
	public DslCommand(string cmdText)
		: this(cmdText, null, null)
	{ }

	public DslCommand(string cmdText, DslConnection connection)
	...
	public static DslCommand CreateWithTypeCoercions(Type[] expectedColumnTypes)
	{
		var result = new DslCommand
		{
	...
	public new DslParameter CreateParameter()
	{
	...
	public new DslDataReader ExecuteReader() => ExecuteReader(CommandBehavior.Default);
	public new Task<DslDataReader> ExecuteReaderAsync() => ExecuteReaderAsync(CommandBehavior.Default);
	public new Task<DslDataReader> ExecuteReaderAsync(CancellationToken cancellationToken) => ExecuteReaderAsync(CommandBehavior.Default, cancellationToken);

	public new DslDataReader ExecuteReader(CommandBehavior behavior)
	{
		CheckCommand();
		...
		_activeReader = new DslDataReader(this, _connection, behavior);
		return _activeReader;
	}
	public new Task<DslDataReader> ExecuteReaderAsync(CommandBehavior behavior) => ExecuteReaderAsync(behavior, CancellationToken.None);
	public new async Task<DslDataReader> ExecuteReaderAsync(CommandBehavior behavior, CancellationToken cancellationToken)
	{
		CheckCommand();
	...
	void IDslPreparedCommand.Release() => Release();
	...
	Task IDslPreparedCommand.ReleaseAsync(CancellationToken cancellationToken) => ReleaseAsync(cancellationToken);
	...
	void IDslPreparedCommand.TransactionCompleted() => TransactionCompleted();
	...
	Task IDslPreparedCommand.TransactionCompletedAsync(CancellationToken cancellationToken) => TransactionCompletedAsync(cancellationToken);
	...
	private void UpdateParameterValues(Descriptor descriptor)
	{
		...
		for (var i = 0; i < descriptor.Count; i++)
		{
			...
			if (_namedParameters.Count > 0)
			{
				...
				if (index == -1)
				{
					Diag.Dug(true, $"Must declare the variable '{_namedParameters[i]}'.");
					throw DslException.Create($"Must declare the variable '{_namedParameters[i]}'.");
				}
			}
				...
				else
				{
					...
					switch (parameter.DbDataType)
					{
						...
						case DbDataType.Guid:
							if (commandParameter.InternalValue is not Guid && commandParameter.InternalValue is not byte[])
							{
	...
	private void Prepare(bool returnsSet)
	{
		...
		if (_transaction == null)
		{
			...
			else
			{
				...
				_transaction = new DslTransaction(_connection, _connection.ConnectionOptions.IsolationLevel);
	...
	private void ExecuteCommand(CommandBehavior behavior, bool returnsSet)
	{
			...
			behavior == CommandBehavior.Default)
		{
			...
			{
				Diag.Dug(true, "Must declare command parameters.");
				throw DslException.Create("Must declare command parameters.");
			}
	...
	private void CheckCommand()
	{
		...
		DslConnection.EnsureOpen(_connection);
		...
		{
			Diag.Dug(true, "Execute requires the Command object to have a Transaction object when the Connection object assigned to the command is in a pending local transaction. The Transaction property of the Command has not been initialized.");
		...
		{
			Diag.Dug(true, "Command Connection is not equal to Transaction Connection.");
		...
		{
			Diag.Dug(true, "The command text for this Command has not been set.");
