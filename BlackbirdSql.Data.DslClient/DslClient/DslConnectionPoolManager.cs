...
sealed class DslConnectionPoolManager : IDisposable
{
	internal static DslConnectionPoolManager Instance { get; private set; }

	sealed class Item
	{
		public long Created { get; private set; }
		public DslConnectionInternal Connection { get; private set; }

		public Item(long created, DslConnectionInternal connection)
		{
			Created = created;
	...
	sealed class Pool : IDisposable
	{
		...
		public Pool(ConnectionString connectionString)
		{
			...
			_busy = new List<DslConnectionInternal>();
		}
		...
		public DslConnectionInternal GetConnection(out bool createdNew)
		{
			DslConnectionInternal connection;
			lock (_syncRoot)
			{
				CheckDisposedImpl();
		...
		public void ReleaseConnection(DslConnectionInternal connection, bool returnToAvailable)
		{
			lock (_syncRoot)
			{
				CheckDisposedImpl();
		...
		DslConnectionInternal GetOrCreateConnectionImpl(out bool createdNew)
		{
			if (_available.Any())
			{
				createdNew = false;
			...
			else
			{
				...
				return new DslConnectionInternal(_connectionString);
			}
		}
	...
	static DslConnectionPoolManager()
	{
		Instance = new DslConnectionPoolManager();
		ShutdownHelper.RegisterPoolCleanup(Instance.Dispose);
	}

	DslConnectionPoolManager()
	{
		_disposed = 0;
	...
	internal void Release(DslConnectionInternal connection, bool returnToAvailable)
	{
		CheckDisposed();
	...
	void CheckDisposed()
	{
		if (Volatile.Read(ref _disposed) == 1)
			throw new ObjectDisposedException(nameof(DslConnectionPoolManager));
	}
}
