...
internal class RemoteEvent
{
	...
	void EnsureEventsCollection(ICollection<string> events)
	{
		...
		if (events.Count == 0)
		{
			ex = new ArgumentOutOfRangeException(nameof(events), "Need to provide at least one event.");
			Diag.Dug(ex);
			throw ex;
		}
		if (events.Any(x => x.Length > MaxEventNameLength))
		{
			ex = new ArgumentOutOfRangeException(nameof(events), $"Some events are longer than {MaxEventNameLength}.");
			Diag.Dug(ex);
			throw ex;
		}
		if (BuildEpb(events.ToList(), _ => default).ToArray().Length > MaxEpbLength)
		{
			ex = new ArgumentOutOfRangeException(nameof(events), $"Whole events buffer is bigger than {MaxEpbLength}.");
			Diag.Dug(ex);
			throw ex;
		}
	}
