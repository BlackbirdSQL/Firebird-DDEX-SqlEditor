...
class DecimalCodec
{
	class DecimalFormat
	{
		...
		public void ValidateByteLength(byte[] decBytes)
		{
			...
			{
				Diag.Dug(true, nameof(decBytes), $"{nameof(decBytes)} argument must be {FormatByteLength} bytes.");
	...
	public FbDecFloat ParseBytes(byte[] decBytes)
	{
		...
		switch (decimalType)
		{
			...
			default:
				Diag.Dug(true, "Argument out of range");
	...
	static byte GetSpecialBits(DecimalType decimalType)
	{
		...
		{
			case DecimalType.Finite:
				InvalidOperationException ex = new InvalidOperationException($"{nameof(DecimalType)} {nameof(DecimalType.Finite)} has no special bits.");
				Diag.Dug(ex);
				throw ex;
			case DecimalType.Infinity:
				result = Infinity0;
			...
			case DecimalType.SignalingNaN:
				...
				break;
			default:
				ArgumentOutOfRangeException exArg = new ArgumentOutOfRangeException();
				Diag.Dug(exArg);
				throw exArg;
		}
