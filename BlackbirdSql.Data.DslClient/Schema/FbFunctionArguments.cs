...
internal class FbFunctionArguments : FbSchema
{
	...
	protected override void ProcessResult(DataTable schema)
	{
		 ...
		 * So we spend hours setting up pretty mappings in the object support xml then cannot use them
		 * in the data view. Surely that cannot be??
		 * Overall the DataViewSupport schema is disappointingly weak and now not supported in .NET.
		 * There also doesn't seem to be a way to specify an editor/viewer for a property in the
		 * property window.
		 * This makes no sense. Some of us aren't hardcore code first enthusiasts!
		 * 
		 */
		...
		foreach (DataRow row in schema.Rows)
		{
			...
			var dbType = (DslDbType)TypeHelper.GetDbDataTypeFromBlrType(blrType, subType, scale);
			row["PARAMETER_DATA_TYPE"] = TypeHelper.GetDataTypeName((DbDataType)dbType).ToLowerInvariant();

			if (dbType == DslDbType.Char || dbType == DslDbType.VarChar)
			{
			...
			if (dbType == DslDbType.Binary || dbType == DslDbType.Text)
			{
			...
			if ((dbType == DslDbType.Decimal || dbType == DslDbType.Numeric) &&
			...
			if (row["ARGUMENT_NAME"] == DBNull.Value)
				row["PSEUDO_NAME"] = row["FUNCTION_NAME"];
			else
