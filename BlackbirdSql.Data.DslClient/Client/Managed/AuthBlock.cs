...
using BlackbirdSql.Data.Client.Managed.Srp;
using BlackbirdSql.Data.Client.Managed.Sspi;
...
sealed class AuthBlock
{
	...
	public IResponse ProcessContAuthResponse()
	{
		...
		{
			Diag.Dug(true, $"Unexpected response ({operation}).");
	...
	public void SendWireCryptToBuffer()
	{
		...
		Connection.Xdr.Write(DslNetworkHandlingWrapper.EncryptionName);
	...
	public async ValueTask SendWireCryptToBufferAsync(CancellationToken cancellationToken = default)
	{
		...
		await Connection.Xdr.WriteAsync(DslNetworkHandlingWrapper.EncryptionName, cancellationToken).ConfigureAwait(false);
	...
	public void Start(byte[] serverData, string acceptPluginName, bool isAuthenticated, byte[] serverKeys)
	{
		...
		{
			Diag.Dug(true, $"Not supported plugin '{AcceptPluginName}'.");
