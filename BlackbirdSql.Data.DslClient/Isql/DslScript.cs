...
//$OriginalAuthors = Abel Eduardo Pereira, Jiri Cincura (jiri@cincura.net)
...
public class DslScript
{
	...
	DslStatementCollection _results;
	...
	/// </summary>
	public DslStatementCollection Results
	{
		get { return _results; }
	}
	...
	/// </summary>
	public static DslScript LoadFromFile(string fileName)
	{
		return new DslScript(File.ReadAllText(fileName));
	}

	public DslScript(string script)
	{
		if (script == null)
			throw new ArgumentNullException();

		_results = new DslStatementCollection();
	...
	public int Parse()
	{
		...
		foreach (var statement in _parser.Parse())
		{
			...
			if (statement.Text.Trim() != string.Empty)
			{
				...
				{
					Diag.Dug(true, $"Both {nameof(UnknownStatementEventArgs.Handled)} and {nameof(UnknownStatementEventArgs.Ignore)} should not be set.");
				...
				{
					Diag.Dug(true, $"The type of the SQL statement could not be determined. See also {nameof(UnknownStatement)} event.{Environment.NewLine}Statement: {statement.Text}.");
