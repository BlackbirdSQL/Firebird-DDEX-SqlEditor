...
//$OriginalAuthors = Abel Eduardo Pereira, Jiri Cincura (jiri@cincura.net), Olivier Metod
...
public sealed class DslBatchExecution
{
	...
	DslStatementCollection _statements;
	DslConnection _sqlConnection;
	DslTransaction _sqlTransaction;
	ConnectionStringBuilder _connectionString;
	DslCommand _sqlCommand;
	...
	/// </summary>
	public DslStatementCollection Statements
	{
	...
	/// connection.
	/// </summary>
	/// <param name="sqlConnection">A <see cref="DslConnection"/> object.</param>
	public DslBatchExecution(DslConnection sqlConnection = null)
	{
		_statements = new DslStatementCollection();
		if (sqlConnection == null)
		{
			_sqlConnection = new DslConnection(); // do not specify the connection string
			_connectionString = new ConnectionStringBuilder();
		}
		else
		{
			_sqlConnection = sqlConnection;
			_connectionString = new ConnectionStringBuilder(sqlConnection.ConnectionString);
		}
	}

	/// <summary>
	/// Appends SQL statements from <see cref="DslScript"/> instance. <see cref="DslScript.Parse"/> should be already called.
	/// </summary>
	/// <param name="isqlScript">A <see cref="DslScript"/> object.</param>
	public void AppendSqlStatements(DslScript isqlScript)
	{
	...
	public void Execute(bool autoCommit = true)
	{
		...
		{
			Diag.Dug(true, "There are no commands for execution.");
		...
		foreach (var statement in Statements)
		{
				...
				statement.StatementType == SqlStatementType.SetSQLDialect))
			{
				...
				{
					_sqlTransaction = _sqlConnection.BeginTransaction(DslTransaction.DefaultIsolationLevel, null);
				}
			...
			try
			{
				switch (statement.StatementType)
				{
					...
					case SqlStatementType.DropDatabase:
						...
						DslConnection.DropDatabase(_connectionString.ToString());
					...
					case SqlStatementType.Disconnect:
						...
						DslConnection.ClearPool(_sqlConnection);
						_requiresNewConnection = false;
			...
			catch (Exception ex)
			{
				...
				var message = string.Format("An exception was thrown when executing command: {1}.{0}Batch execution aborted.{0}The returned message was: {2}.",
					...
					ex.Message);
				Diag.Dug(ex);
				throw DslException.Create(message, ex);
			}
		}
	...
	public async Task ExecuteAsync(bool autoCommit = true, CancellationToken cancellationToken = default)
	{
		...
		foreach (var statement in Statements)
		{
				...
				statement.StatementType == SqlStatementType.SetSQLDialect))
			{
				...
				if (_sqlTransaction == null && !(statement.StatementType == SqlStatementType.Commit || statement.StatementType == SqlStatementType.Rollback))
				{
					_sqlTransaction = await _sqlConnection.BeginTransactionAsync(DslTransaction.DefaultIsolationLevel, null, cancellationToken).ConfigureAwait(false);
				}
				_sqlCommand.Transaction = _sqlTransaction;
			}
			...
			try
			{
				switch (statement.StatementType)
				{
					...
					case SqlStatementType.DropDatabase:
						...
						await DslConnection.DropDatabaseAsync(_connectionString.ToString(), cancellationToken).ConfigureAwait(false);
						_requiresNewConnection = true;
	...
	private void ConnectToDatabase(string connectDbStatement)
	{
		...
		using (var enumerator = parser.Parse().GetEnumerator())
		{
			...
			{
				Diag.Dug(true, "Malformed isql CONNECT statement. Expected keyword CONNECT but something else was found.");
			...
			while (enumerator.MoveNext())
			{
				switch (enumerator.Current.Text.ToUpperInvariant())
				{
					...
					default:
						Diag.Dug(true, "Unexpected token '" + enumerator.Current.Text + "' on isql CONNECT statement.");
	...
	private void CreateDatabase(string createDatabaseStatement)
	{
		...
		using (var enumerator = parser.Parse().GetEnumerator())
		{
			...
			{
				Diag.Dug(true, "Malformed isql CREATE statement. Expected keyword CREATE but something else was found.");
		...
		DslConnection.CreateDatabase(_connectionString.ToString(), pageSize: pageSize);
		_requiresNewConnection = true;
		ProvideConnection();
	}
	/// <summary>
	...
	private async Task CreateDatabaseAsync(string createDatabaseStatement, CancellationToken cancellationToken = default)
	{
		...
		await DslConnection.CreateDatabaseAsync(_connectionString.ToString(), pageSize: pageSize, cancellationToken: cancellationToken).ConfigureAwait(false);
		_requiresNewConnection = true;
		await ProvideConnectionAsync(cancellationToken).ConfigureAwait(false);
	}
	...
	private void SetAutoDdl(string setAutoDdlStatement, ref bool autoCommit)
	{
		...
		using (var enumerator = parser.Parse().GetEnumerator())
		{
			...
			if (enumerator.Current.Text.ToUpperInvariant() != "SET")
			{
				Diag.Dug(true, "Malformed isql SET statement. Expected keyword SET but something else was found.");
			...
			if (enumerator.MoveNext())
			{
				...
				{
					Diag.Dug(true, "Expected the ON or OFF but something else was found.");
	...
	private DslCommand ProvideCommand()
	{
		if (_sqlCommand == null)
		{
			_sqlCommand = new DslCommand();
		}
	...
	private async Task<DslCommand> ProvideCommandAsync(CancellationToken cancellationToken = default)
	{
		if (_sqlCommand == null)
		{
			_sqlCommand = new DslCommand();
	...
	private DslConnection ProvideConnection()
	{
		if (_requiresNewConnection)
		{
			...
			_sqlConnection = new DslConnection(_connectionString.ToString());
		}
	...
	private async Task<DslConnection> ProvideConnectionAsync(CancellationToken cancellationToken = default)
	{
		if (_requiresNewConnection)
		{
			if (_sqlConnection != null && _sqlConnection.State != ConnectionState.Closed)
	...
	private void OnCommandExecuting(DslCommand sqlCommand, SqlStatementType statementType)
	{
	...
	/// </summary>
	/// <param name="commandText">The <see cref="DslCommand.CommandText"/> of the executed SQL command.</param>
	/// <param name="dataReader">The <see cref="DslDataReader"/> instance with the returned data. If the
	...
	private void OnCommandExecuted(DslDataReader dataReader, string commandText, SqlStatementType statementType, int rowsAffected)
	{
