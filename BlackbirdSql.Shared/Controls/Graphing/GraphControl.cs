...
using BlackbirdSql.Shared.Controls.Graphing.Widgets
using BlackbirdSql.Shared.Controls.Widgets;
using BlackbirdSql.Shared.Ctl.Enums;
using BlackbirdSql.Shared.Ctl.Exceptions;
using BlackbirdSql.Shared.Ctl.Interfaces;
...
public class GraphControl : GraphCtrl
{
	...
	{
		int num = 0;
		foreach (BlackbirdSql.Shared.Controls.Graphing.NodeDisplay item in NodesZOrder.Cast<NodeDisplay>())
		{
	...
	private void SetNodePositionRecursive(BlackbirdSql.Shared.Controls.Graphing.NodeDisplay node, int spacingY, int x, int y)
	{
	...
	private void SetNodeXPostitionRecursive(BlackbirdSql.Shared.Controls.Graphing.NodeDisplay node, int x)
	{
		node.Position = new Point(x, 0);
		int num = node.InitializeEdges() + C_PaddingX;
		x += num;
		node.MaxChildrenXPosition = node.BoundingRect.Right;
		foreach (BlackbirdSql.Shared.Controls.Graphing.NodeDisplay child in node.Children)
		{
	...
	private void SetNodeYPositionRecursive(BlackbirdSql.Shared.Controls.Graphing.NodeDisplay node, GraphNodeLayoutHelper layoutHelper, int spacingY, int y)
	{
		...
		if (node.Parent != null && node.Parent.Position.Y != node.Position.Y)
		{
			...
			{
				num2 = Math.Min(num2, (edgesRelated.Current as BlackbirdSql.Shared.Controls.Graphing.EdgeDisplay).GetMidpoint());
			}
			xPosition = num2;
		}
	...
	{
		return (Microsoft.AnalysisServices.Graphing.NodeDisplay)Activator.CreateInstance(((BlackbirdSql.Shared.Controls.Graphing.Node)originalNode).Operation.DisplayNodeType, this, this, NodeDisplayPropertiesDefault, NodesZOrder, originalNode);
	}
	...
	{
		return new BlackbirdSql.Shared.Controls.Graphing.EdgeDisplay(this, this, iedgeOrig, EdgeDisplayPropertiesDefault, EdgesZOrder, inodedisplayFrom, inodedisplayTo);
	}
	...
	private void MoveSelectionRight()
	{
		...
		if (CurrentSelection is BlackbirdSql.Shared.Controls.Graphing.NodeDisplay)
		{
			BlackbirdSql.Shared.Controls.Graphing.NodeDisplay nodeDisplay = CurrentSelection as BlackbirdSql.Shared.Controls.Graphing.NodeDisplay;
			...
			while (edgesRelated.MoveNext())
			{
				...
				{
					SelectEdge(edgesRelated.Current as BlackbirdSql.Shared.Controls.Graphing.EdgeDisplay);
					break;
				}
			}
		}
		else if (CurrentSelection is BlackbirdSql.Shared.Controls.Graphing.EdgeDisplay)
		{
			BlackbirdSql.Shared.Controls.Graphing.EdgeDisplay edgeDisplay = CurrentSelection as BlackbirdSql.Shared.Controls.Graphing.EdgeDisplay;
	...
	private void MoveSelectionDown()
	{
		...
		{
			if (CurrentSelection is not BlackbirdSql.Shared.Controls.Graphing.EdgeDisplay)
			{
				return;
			}
			BlackbirdSql.Shared.Controls.Graphing.EdgeDisplay edgeDisplay = CurrentSelection as BlackbirdSql.Shared.Controls.Graphing.EdgeDisplay;
			BlackbirdSql.Shared.Controls.Graphing.NodeDisplay nodeFrom = edgeDisplay.NodeFrom;
		...
		else
		{
			...
			while (edgesRelated.MoveNext())
			{
				if (edgesRelated.Current == edgeDisplay)
				{
					if (edge != null)
					{
						SelectEdge(edge as BlackbirdSql.Shared.Controls.Graphing.EdgeDisplay);
					}
					break;
				}
	...
	private void HandleToolTip(object displayObject)
	{
		if (displayObject != toolTip.CurrentTag)
		{
			...
			if (displayObject is BlackbirdSql.Shared.Controls.Graphing.NodeDisplay nodeDisplay)
			{
				string text = nodeDisplay.DisplayName.Split('\n')[0];
				toolTip.SetToolTip(this, text);
			}
			else if (displayObject is BlackbirdSql.Shared.Controls.Graphing.EdgeDisplay)
			{
	...
	{
		if (CurrentSelection is BlackbirdSql.Shared.Controls.Graphing.NodeDisplay nodeDisplay && !string.IsNullOrEmpty(nodeDisplay.HelpKeyword))
		{
