...
public class FbTypeMappingSource : RelationalTypeMappingSource
{
	...
	readonly FbStringTypeMapping _char = new FbStringTypeMapping("CHAR", DbType.StringFixedLength, DslDbType.Char);
	readonly FbStringTypeMapping _varchar = new FbStringTypeMapping("VARCHAR", DbType.String, DslDbType.VarChar);
	readonly FbStringTypeMapping _clob = new FbStringTypeMapping("BLOB SUB_TYPE TEXT", DbType.String, DslDbType.Text);
	...
	readonly FbDateTimeTypeMapping _timestamp = new FbDateTimeTypeMapping("TIMESTAMP", DslDbType.TimeStamp);
	readonly FbDateTimeTypeMapping _date = new FbDateTimeTypeMapping("DATE", DslDbType.Date);
	readonly FbDateOnlyTypeMapping _dateOnly = new FbDateOnlyTypeMapping("DATE");

	readonly FbTimeSpanTypeMapping _timeSpan = new FbTimeSpanTypeMapping("TIME", DslDbType.Time);
		...
		if (clrType != null)
		{
			...
			if (clrType == typeof(string))
			{
				...
				else
				{
					if (!isFixedLength)
					{
						return new FbStringTypeMapping($"VARCHAR({size})", DbType.String, DslDbType.VarChar, size, isUnicode);
					}
					else
					{
						return new FbStringTypeMapping($"CHAR({size})", DbType.StringFixedLength, DslDbType.Char, size, isUnicode);
					}
				}
			}
