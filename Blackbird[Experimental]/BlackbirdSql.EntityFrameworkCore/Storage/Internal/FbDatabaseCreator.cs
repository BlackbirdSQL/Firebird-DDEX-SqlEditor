...
public class FbDatabaseCreator : RelationalDatabaseCreator
{
	...
	{
		DslConnection.CreateDatabase(_connection.ConnectionString, pageSize: 16384);
	...
	{
		await DslConnection.CreateDatabaseAsync(_connection.ConnectionString, pageSize: 16384, cancellationToken: cancellationToken).ConfigureAwait(false);
	...
	{
		DslConnection.ClearPool((DslConnection)_connection.DbConnection);
		DslConnection.DropDatabase(_connection.ConnectionString);
	}
	public override Task DeleteAsync(CancellationToken cancellationToken = default)
	{
		DslConnection.ClearPool((DslConnection)_connection.DbConnection);
		return DslConnection.DropDatabaseAsync(_connection.ConnectionString, cancellationToken);
	}
