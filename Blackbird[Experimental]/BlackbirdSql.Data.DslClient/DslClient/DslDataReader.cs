...
using BlackbirdSql.Data.Types;



namespace BlackbirdSql.Data.DslClient;

public sealed class DslDataReader : DbDataReader
{
	#region Constants
	...
	internal DslDataReader()
		: base()
	{ }

	internal DslDataReader(DslCommand command, DslConnection connection, CommandBehavior commandBehavior)
	{
	...
	public override DataTable GetSchemaTable()
	{
		...
		var schemaCmd = new DslCommand(GetSchemaCommandText(), _command.Connection, _command.Connection.InnerConnection.ActiveTransaction);
		try
		{
			schemaCmd.Parameters.Add("@TABLE_NAME", DslDbType.Char, 31);
			schemaCmd.Parameters.Add("@COLUMN_NAME", DslDbType.Char, 31);
			schemaCmd.Prepare();
	...
	private DslDbType GetProviderType(int i)
	{
		return (DslDbType)_fields[i].DbDataType;
	}
	...
	private static bool IsReadOnly(DslDataReader r)
	{
		return IsExpression(r);
	}

	public static bool IsExpression(DslDataReader r)
	{
		/* [0] = COMPUTED_BLR
		 * [1] = COMPUTED_SOURCE
		 */
