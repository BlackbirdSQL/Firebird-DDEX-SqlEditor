		...
		while ((type = buffer[pos++]) != IscCodes.isc_info_end)
		{
			...
			switch (type)
			{
				case IscCodes.isc_info_error:
					Diag.Dug(true, "Received error response.");
					throw DslException.Create("Received error response.");
				...
				case IscCodes.isc_info_blackbird_version:
					{
				...
				case IscCodes.isc_info_db_class:
					{
						...
						switch (serverClass)
						{
							case IscCodes.isc_info_db_class_classic_access:
								param = "CLASSIC SERVER";
								break;
							case IscCodes.isc_info_db_class_server_access:
								param = "SUPER SERVER";
								break;
							default:
								ex = new ArgumentOutOfRangeException(nameof(serverClass), $"{nameof(serverClass)}={serverClass}");
								Diag.Dug(ex);
								throw ex;
						}
						info.Add(param);
				...
				case IscCodes.fb_info_replica_mode:
					{
						...
						switch (mode)
						{
							...
							case 2:
								...
								break;
							default:
								ex = new ArgumentOutOfRangeException(nameof(mode), $"{nameof(mode)}={mode}");
								Diag.Dug(ex);
								throw ex;
						}
						info.Add(param);
				...
				default:
					Diag.Dug(true, $"Argument out of range: {nameof(type)}={type}");
