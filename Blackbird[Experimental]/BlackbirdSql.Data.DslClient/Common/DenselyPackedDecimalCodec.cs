...
using static BlackbirdSql.Data.Common.BitHelpers;
...
class DenselyPackedDecimalCodec
{
	...
	//@formatter:off
	// Generated using org.blackbirdsql.decimal.generator.GenerateLookupTable
	...
	public DenselyPackedDecimalCodec(int numberOfDigits)
	{
		...
		{
			ArgumentOutOfRangeException ex = new ArgumentOutOfRangeException(nameof(numberOfDigits), $"{nameof(numberOfDigits)} must be of form n * 3 + 1 with n > 0, was {numberOfDigits}.");
			Diag.Dug(ex);
			throw ex;
		}
	...
	public BigInteger DecodeValue(int signum, int firstDigit, byte[] decBytes, int lsbIndex)
	{
		...
		{
			ArgumentOutOfRangeException ex = new ArgumentOutOfRangeException(nameof(firstDigit), $"{nameof(firstDigit)} must be in range 0 <= firstDigit <= 9, was {firstDigit}.");
			Diag.Dug(ex);
			throw ex;
		}
