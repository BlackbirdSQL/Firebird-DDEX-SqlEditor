...
//$OriginalAuthors = Dean Harding, Jiri Cincura (jiri@cincura.net)
...
using BlackbirdSql.Data.Client.Native.Handles;
using BlackbirdSql.Data.Common;

namespace BlackbirdSql.Data.Client.Native;
...
/// </summary>
internal static class DslClientFactory
{
	...
	/// </summary>
	static DslClientFactory()
	{
		cache = new Dictionary<string, IFbClient>();
		cacheLock = new ReaderWriterLockSlim(LockRecursionPolicy.NoRecursion);
		injectionTypes = new HashSet<Type>(typeof(DslClientFactory).Assembly.GetTypes()
			.Where(x => !x.IsAbstract && !x.IsInterface)
			.Where(x => typeof(IDslHandle).IsAssignableFrom(x))
	...
	private static void GenerateMethod(TypeBuilder tb, MethodInfo mi, string dllName)
	{
		...
		EmitClientInjectionToBlackbirdHandleOjects(mi.ReturnType, pis, il);
	...
	private static void EmitClientInjectionToBlackbirdHandleOjects(
		Type returnType,
		...
		if (injectProperties)
		{
			...
			for (var i = 0; i < pis.Count; i++)
			{
				if (injectionTypes.Contains(pis[i].ParameterType))
				{
					...
					il.Emit(OpCodes.Callvirt, typeof(IDslHandle).GetMethod("SetClient"));
					il.Emit(OpCodes.Nop);
				}
			}
