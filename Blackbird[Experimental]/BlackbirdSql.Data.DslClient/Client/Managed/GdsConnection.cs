...
internal sealed class GdsConnection
{
	...
	private DslNetworkHandlingWrapper _blackbirdNetworkHandlingWrapper;
	...
	public bool ConnectionBroken => _blackbirdNetworkHandlingWrapper?.IOFailed ?? false;
	...
	public void Connect()
	{
		...
		try
		{
			...
			_blackbirdNetworkHandlingWrapper = new DslNetworkHandlingWrapper(new DataProviderStreamWrapper(_networkStream));
			Xdr = new XdrReaderWriter(_blackbirdNetworkHandlingWrapper, Charset);
		}
	...
	public void Identify(string database)
	{
		...
		try
		{
			...
			{
				Diag.Dug(true, "Exception IscCodes.isc_connect_reject");
	...
	internal IResponse ProcessOperation(int operation)
	{
		...
		switch (operation)
		{
			...
			default:
				Diag.Dug(true, $"Argument out of range: {nameof(operation)}={operation}");
