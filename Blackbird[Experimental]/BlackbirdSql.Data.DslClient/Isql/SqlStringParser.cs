...
class SqlStringParser
{
	...
	public IEnumerable<DslStatement> Parse()
	{
		var lastYield = 0;
		var index = 0;
		...
		Continue:
			...
			else
			{
				foreach (var token in Tokens)
				{
					...
					{
						index += token.Length;
						yield return new DslStatement(_source.Substring(lastYield, index - lastYield - token.Length), rawResult.ToString());
						lastYield = index;
						rawResult.Clear();
						goto Continue;
					}
		...
		if (index >= _sourceLength)
		{
			...
			yield return new DslStatement(parsed, rawResult.ToString());
			lastYield = _sourceLength;
			rawResult.Clear();
		}
		else
		{
			yield return new DslStatement(_source.Substring(lastYield, index - lastYield), rawResult.ToString());
			lastYield = index;
			rawResult.Clear();
		}
	}
