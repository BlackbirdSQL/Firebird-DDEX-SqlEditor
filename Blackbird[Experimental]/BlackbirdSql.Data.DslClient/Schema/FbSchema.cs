...
internal abstract class FbSchema
{
	...
	public DataTable GetSchema(DslConnection connection, string collectionName, string[] restrictions)
	{
		...
		try
		{
			using (var adapter = new DslDataAdapter(command))
			{
				try
				{
	...
	public async Task<DataTable> GetSchemaAsync(DslConnection connection, string collectionName, string[] restrictions, CancellationToken cancellationToken = default)
	{
		var dataTable = new DataTable(collectionName);
		var command = BuildCommand(connection, collectionName, ParseRestrictions(restrictions));
		try
	...
	protected DslCommand BuildCommand(DslConnection connection, string collectionName, string[] restrictions)
	{
		...
		var command = new DslCommand(builder.ToString(), connection, transaction);
			...
			for (var i = 0; i < restrictions.Length; i++)
			{
				...
				if (restrictions[i] != null)
				{
					...
					if (!rname.EndsWith("Catalog") && !rname.EndsWith("Schema") && rname != "TableType")
					{
						...
						command.Parameters.Add(pname, DslDbType.VarChar, 255).Value = restrictions[i];
					}
				}
			}
		}
	...
	private void SetMajorVersionNumber(DslConnection connection)
	{
