...
// =========================================================================================================
//									TMemoryPtr Class Template Definitions
//
...
// ---------------------------------------------------------------------------------
template<typename T> T* TMemoryPtr<T>::Ptr::get()
{
...
// ---------------------------------------------------------------------------------
template<typename T> size_t TMemoryPtr<T>::Size::get()
{
	return _MemSize;
}
...
// Linker hack.
// --------------------------------------
template<typename T> T TMemoryPtr<T>::GetDefault(int i)
{
	if (_MemBlock == __nullptr)
	{
...
template<typename T> void TMemoryPtr<T>::SetDefault(int i, T value)
{
	if (i < 0)
	{
...
// Linker hack.
// ---------------------------------------------------------------------------------
template<typename T> T* TMemoryPtr<T>::GetDefault(int offset, size_t len)
{
	if (_MemBlock == __nullptr)
	{
...
template<typename T> void TMemoryPtr<T>::SetDefault(int offset, size_t len, T* value)
{
	if (offset < 0)
	{
...
/// Default .ctor.
// ---------------------------------------------------------------------------------
template<typename T> TMemoryPtr<T>::TMemoryPtr()
{
}
...
// ---------------------------------------------------------------------------------
template<typename T> TMemoryPtr<T>::TMemoryPtr(size_t size)
{
	Malloc(size);
}
...
// ---------------------------------------------------------------------------------
template<typename T> TMemoryPtr<T>::TMemoryPtr(T* src, size_t len)
{
	MemCpy(src, len);
}
...
// ---------------------------------------------------------------------------------
template<typename T> TMemoryPtr<T>::TMemoryPtr(String^ src)
{
	MemCpy(src);
}
...
// ---------------------------------------------------------------------------------
template<typename T> void TMemoryPtr<T>::Malloc(size_t size)
{
	Realloc(size);
}
...
// ---------------------------------------------------------------------------------
template<typename T> void TMemoryPtr<T>::Free()
{
	if (_MemBlock != __nullptr)
	{
		free(_MemBlock);
...
// ---------------------------------------------------------------------------------
template<typename T> void TMemoryPtr<T>::Realloc(size_t size)
{
	if (_MemBlock != __nullptr)
	{
		if (_MemSize < size)
...
// ---------------------------------------------------------------------------------
template<typename T> void TMemoryPtr<T>::MemCpy(T* src, size_t len)
{
...
// ---------------------------------------------------------------------------------
template<typename T> void TMemoryPtr<T>::MemCpy(String^ src)
{
