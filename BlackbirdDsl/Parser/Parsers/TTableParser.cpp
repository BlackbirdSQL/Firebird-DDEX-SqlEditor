...
StringCell^ TTableParser::GetReservedType(StringCell^ token)
{
...
StringCell^ TTableParser::GetConstantType(StringCell^ token)
{
...
StringCell^ TTableParser::GetOperatorType(StringCell^ token)
{
...
void TTableParser::Clear(StringCell^% expr, SysStr^% base_expr, SysStr^% category)
{
...
StringCell^ TTableParser::MoveLike(StringCell^ root)
{
...
StringCell^ TTableParser::Parse(StringCell^ root)
{
	...
	for each (ReplicaKeyPair(StringCell^) pair in parserNode->ReplicaKeyEnumerator)
	{
		...
		else if (upper == "PARTITION")
		{
			if (prevCategory == "CREATE_DEF")
			{
				...
				cell = ParserInstance(WPartitionOptionsParser)->Parse(cell);
		...
		else
		{
			...
			else if (currCategory == "")
			{
				...
				if (prevCategory == "TABLE_NAME" && upper[0] == '(' && upper->EndsWith(")"))
				{
					...
					coldef = (gcnew WCreateDefinitionParser(Options))->Execute(RemoveParenthesis(trim));
