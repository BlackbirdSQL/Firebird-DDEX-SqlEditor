		...
		else
		{
			if (currCategory == "COLUMN_DEF")
			{
				if (upper->StartsWith("(") && upper->EndsWith(")"))
				{
					...
					cols = (gcnew IndexColumnListParser(_Options))->Execute(RemoveParenthesis(trim));
