...
*/
StringCell^ SelectExpressionParser::Parse(StringCell^ root)
{
	if (root->Count == 0)
		return NullCell;
	/*
	...
	*/
	...
	stripped = ParserInstance(ExpressionListParser)->Parse(stripped);
	...
	processed = ParserInstance(ExpressionListParser)->Parse(root);
