...
StringCell^ FromParser::InitParseInfo(StringCell^ parseInfo)
{
	// first init
	if (parseInfo->IsNull)
	{
...
StringCell^ FromParser::ParseFromExpression(StringCell^ parseInfo)
{
	...
	if (!parseInfo->IsUnpopulated["ref_expr"])
	{
		...
		if (parseInfo["ref_type"] == "USING")
		{
			...
			cell = (gcnew ColumnListParser(Options))->Execute(RemoveParenthesis(unparsed[0]));
		...
		{
			ref = ParserInstance(ExpressionListParser)->Parse(unparsed);
		}
...
StringCell^ FromParser::Parse(StringCell^ root)
{
