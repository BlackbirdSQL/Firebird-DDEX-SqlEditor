...
internal sealed class SqlGenerator : DbExpressionVisitor<ISqlFragment>
{
	...
	private bool IsParentAJoin
	{
		...
		get { return _isParentAJoinStack.Count != 0 && _isParentAJoinStack.Peek(); }
	}
	...
	readonly Dictionary<string, string> _shortenedNames = new Dictionary<string, string>();
	...
	internal static string GenerateSql(DbCommandTree tree, out List<DbParameter> parameters, out CommandType commandType)
	{
		...
		Diag.Dug(true, "Unrecognized command tree type");
	...
	public override ISqlFragment Visit(DbApplyExpression e)
	{
		...
		Diag.Dug(true, $"{joinString} statement is not supported in Firebird.");
	...
	public override ISqlFragment Visit(DbCaseExpression e)
	{
		...
		if (e.Else != null && e.Else is not DbNullExpression)
		{
			result.Append(" ELSE ");
		...
		if (MetadataHelpers.TryGetPrimitiveTypeKind(e.ResultType, out var typeKind))
		{
			switch (typeKind)
			{
				...
				case PrimitiveTypeKind.Decimal:
					...
					result.Append(" AS ");
					result.Append(sqlPrimitiveType[..sqlPrimitiveType.IndexOf('(')]);
					result.Append("(");
				...
				case PrimitiveTypeKind.String:
					 ...
					 *	?? (isUnicode ? ProviderManifest.UnicodeVarcharMaxSize : ProviderManifest.AsciiVarcharMaxSize);
					 *	result.Append(FormatString((string)e.Value, isUnicode, length));
					*/
					MetadataHelpers.GetFacetValueOrDefault(e.ResultType, MetadataHelpers.UnicodeFacetName, true);
					// constant is always considered Unicode
					var length = MetadataHelpers.GetFacetValueOrDefault<int?>(e.ResultType, MetadataHelpers.MaxLengthFacetName, null)
						?? ProviderManifest.UnicodeVarcharMaxSize;
	...
	{
		Diag.Dug(true, "The EXCEPT statement is not supported in Firebird.");
	...
	{
		Diag.Dug(true, "The INTERSECT statement is not supported in Firebird.");
	...
	public override ISqlFragment Visit(DbPropertyExpression e)
	{
		...
		if (instanceSql is SymbolPair symbolPair)
		{
			...
			else
			{
				...
				if (symbolPair.Column.Columns.TryGetValue(e.Property.Name, out Symbol value))
				{
					result = new SqlBuilder();
	...
	SqlBuilder VisitAggregate(DbAggregate aggregate, object aggregateArgument)
	{
		...
		if (aggregate is not DbFunctionAggregate functionAggregate)
		{
			Diag.Dug(true, "Not supported");
			throw new NotSupportedException();
		}
		...
		string joinString, DbExpression joinCondition)
	{
		...
		for (var idx = 0; idx < inputCount; idx++)
		{
			...
			_isParentAJoinStack.Push(needsJoinContext);
					...
					else
					{
						...
						if (sqlSelectStatement.FromExtents[0] is JoinSymbol oldJoinSymbol)
						{
							...
							var newJoinSymbol = new JoinSymbol(varName, input.VariableType, oldJoinSymbol.ExtentList)
							{
								...
								IsNestedJoin = true,
								ColumnList = columns,
								FlattenedExtentList = oldJoinSymbol.FlattenedExtentList
							};
	...
	private void HandleFunctionArgumentsDefault(DbFunctionExpression e, SqlBuilder result)
	{
		...
		{
			Diag.Dug(true, "Niladic functions cannot have parameters");
	...
	{
		Diag.Dug(true, "BitwiseNot is not supported by Firebird.");
	...
	{
		Diag.Dug(true, "CurrentUtcDateTime is not supported by Firebird.");
	...
	{
		Diag.Dug(true, "CurrentDateTimeOffset is not supported by Firebird.");
	...
	{
		Diag.Dug(true, "GetTotalOffsetMinutes is not supported by Firebird.");
	...
	private static ISqlFragment HandleCanonicalFunctionCreateDateTime(SqlGenerator sqlgen, DbFunctionExpression e)
	{
		...
		result.Append("CAST('");
		result.Append(e.Arguments[0].ExpressionKind == DbExpressionKind.Constant ? e.Arguments[0].Accept(sqlgen) : "0001"); // year
		result.Append("-");
		result.Append(e.Arguments[1].ExpressionKind == DbExpressionKind.Constant ? e.Arguments[1].Accept(sqlgen) : "01"); // month
		result.Append("-");
		result.Append(e.Arguments[2].ExpressionKind == DbExpressionKind.Constant ? e.Arguments[2].Accept(sqlgen) : "01"); // day
		result.Append(" ");
		result.Append(e.Arguments[3].ExpressionKind == DbExpressionKind.Constant ? e.Arguments[3].Accept(sqlgen) : "00"); // hour
		result.Append(":");
		result.Append(e.Arguments[4].ExpressionKind == DbExpressionKind.Constant ? e.Arguments[4].Accept(sqlgen) : "00"); // minute
		result.Append(":");
	...
	{
		Diag.Dug(true, "CreateDateTimeOffset is not supported by Firebird.");
		...
		List<Symbol> columnList, Dictionary<string, Symbol> columnDictionary, ref string separator)
	{
		if (symbol is JoinSymbol joinSymbol)
		{
			...
			else
			{
				foreach (var joinColumn in joinSymbol.ColumnList)
				{
					...
					if (columnDictionary.TryGetValue(joinColumn.Name, out Symbol value))
					{
						value.NeedsRenaming = true; // the original symbol
		...
		else
		{
			...
			foreach (var property in MetadataHelpers.GetProperties(symbol.Type))
			{
				...
				if (columnDictionary.TryGetValue(recordMemberName, out Symbol value))
				{
	...
	internal static string GetSqlPrimitiveType(TypeUsage type)
	{
		...
		switch (primitiveType.PrimitiveTypeKind)
		{
			...
			case PrimitiveTypeKind.String:
				...
				{
					if (length > (isUnicode ? ProviderManifest.UnicodeVarcharMaxSize : ProviderManifest.AsciiVarcharMaxSize))
					{
			...
			default:
				Diag.Dug(true, "Unsupported EdmType: " + primitiveType.PrimitiveTypeKind);
	...
	{
		if (expression is not DbPropertyExpression propertyExpression)
		{
			return false;
		}

		if (propertyExpression.Instance is not DbVariableReferenceExpression)
		{
			return false;
		}
		return true;
