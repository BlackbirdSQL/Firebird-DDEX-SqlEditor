...
public class EfbMigrationSqlGenerator : MigrationSqlGenerator
{
	readonly IEfbMigrationSqlGeneratorBehavior _behavior;

	string _migrationsHistoryTableName;

	public EfbMigrationSqlGenerator(IEfbMigrationSqlGeneratorBehavior behavior = null)
	{
		_behavior = behavior ?? new EfbDefaultMigrationSqlGeneratorBehavior();
	}
...
#pragma warning restore SYSLIB1045 // Convert to 'GeneratedRegexAttribute'.
	...
	static string ExtractName(string name) => name[(name.LastIndexOf('.') + 1)..];
...
#pragma warning restore SYSLIB1045 // Convert to 'GeneratedRegexAttribute'.
	...
	static string CheckName(string name)
	{
		...
		{
			ArgumentOutOfRangeException ex = new ArgumentOutOfRangeException($"The name '{name}' is longer than Firebird's {LengthLimit} characters limit for object names.");
			Diag.Dug(ex);
			throw ex;
		}
		return name;
	...
	{
		return DbConfiguration.DependencyResolver.GetService<DbProviderFactory>(PackageData.Invariant).CreateConnection();
	}
