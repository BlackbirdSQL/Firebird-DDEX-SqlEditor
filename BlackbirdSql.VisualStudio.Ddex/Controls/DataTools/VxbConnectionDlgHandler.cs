...
// =========================================================================================================
//										VxbConnectionDlgHandler Class
//
// =========================================================================================================
public class VxbConnectionDlgHandler : IBsDataConnectionDlgHandler
{

	// ---------------------------------------------------------------------------------
	#region Constructors / Destructors - VxbConnectionDlgHandler
	// ---------------------------------------------------------------------------------


	public VxbConnectionDlgHandler()
	{
		try
		{
			_ConnectionDlg = (VxbConnectionDlg)Activator.CreateInstance(typeof(VxbConnectionDlg),
	...
	// =========================================================================================================
	#region Fields - VxbConnectionDlgHandler 
	// =========================================================================================================
	...
	private readonly VxbConnectionDlg _ConnectionDlg;
	...
	// =========================================================================================================
	#region Property accessors - VxbConnectionDlgHandler
	// =========================================================================================================
	...
	public ICollection<Guid> AvailableSources
	{
		get
		{
			_AvailableSources ??= new TiDataSourceCollection(this);
			return _AvailableSources;
		}
	}

	private TiDataSourceCollection AvailableDataSourceCollection => _AvailableSources as TiDataSourceCollection;
	...
	public ICollection<Guid> AvailableProviders
	{
		get
		{
			...
			{
				_AvailableProviders.Add(SelectedSource, new TiDataProviderCollection(_ConnectionDlg.SelectedDataSource));
			}
	...
	public Guid SelectedProvider
	{
		...
		set
		{
			...
			if (value != Guid.Empty)
			{
				...
				_ConnectionDlg.SelectedDataProvider = (AvailableProviders as TiDataProviderCollection).Mapping[value];
			}
			else
			{
	...
	public string SafeConnectionString
	{
		get
		{
			...
			if (_ConnectionDlg.SelectedDataProvider != null)
			{
				TiDataConnectionUIProperties uIDataConnectionProperties = ((_ConnectionDlg.SelectedDataSource != _ConnectionDlg.UnspecifiedDataSource) ? (_ConnectionDlg.SelectedDataProvider.CreateConnectionProperties(_ConnectionDlg.SelectedDataSource) as TiDataConnectionUIProperties) : (_ConnectionDlg.SelectedDataProvider.CreateConnectionProperties() as TiDataConnectionUIProperties));
	...
	*/
	...
	// =========================================================================================================
	#region Methods - VxbConnectionDlgHandler
	// =========================================================================================================
	...
	public void AddSources(DataConnectionDialogFilterCallback callback)
	{
		...
		{
			if (value.Guid != TDataSource.FbDataSource.NameClsid)
				continue;
	...
	public void LoadSourceSelection()
	{
		...
		*/
		SelectedSource = TDataSource.FbDataSource.NameClsid;
		SelectedProvider = TDataProvider.BlackbirdSqlDataProvider.NameClsid;
	}
	...
	public void LoadExistingConfiguration(Guid clsidProvider, string connectionString, bool encryptedString)
	{
		...
		{
			ArgumentNullException ex = new("provider");
			Diag.Dug(ex);
			throw ex;
		}
		...
		if (clsidProvider.ToUpper() != TDataProvider.BlackbirdSqlDataProvider.Name)
		{
		...
		IVsDataProvider value = new TVsDataProvider(clsidProvider);
		/*
	...
	public virtual DialogResult ShowDialog(VxbConnectionDlg form)
	{
		...
		try
		{
			return VxbConnectionDlg.Show(form, UiService.GetDialogOwnerWindow());
		}
		finally
		{
	...
	// =========================================================================================================
	#region Event Handling - VxbConnectionDlgHandler
	// =========================================================================================================
	...
	// =========================================================================================================
	#region						Sub-Classes - VxbConnectionDlgHandler
	// =========================================================================================================
	...
	/// </summary>
	// ---------------------------------------------------------------------------------
	private class TiDataSourceCollection(VxbConnectionDlgHandler dialog) : ICollection<Guid>, IEnumerable<Guid>, IEnumerable
	{
		private readonly IDictionary<Guid, TDataSource> _GuidDataSourceMapping = new Dictionary<Guid, TDataSource>();

		private readonly VxbConnectionDlgHandler _DlgHandler = dialog;
