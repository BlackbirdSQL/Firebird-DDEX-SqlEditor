	...
	public sealed class BlackbirdSqlDdexExtension : ControllerAsyncPackage
	{
		...
		private async Task AdviseEventsAsync()
		{
			...
			try
			{
				...
				if (DteSolution == null)
					throw new NullReferenceException("SVsSolution is null");

				if (DocTable == null)
					throw new NullReferenceException("IVsRunningDocumentTable is null");

				if (Controller == null)
					throw new NullReferenceException("IBPackageController is null");
			}
			catch (Exception ex)
			{
				Diag.Dug(ex);
		...
		public override void RegisterOptionsEventHandlers(IBAsyncPackage.SettingsSavedDelegate onSettingsSavedDelegate)
		{
			...
			{
				Diag.Dug(ex, "Failed to subscribe to Option events");
			}
		}
