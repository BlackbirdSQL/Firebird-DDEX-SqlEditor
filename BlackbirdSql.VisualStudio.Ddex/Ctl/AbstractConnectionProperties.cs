...
/// </summary>
public abstract class AbstractConnectionProperties : DataSiteableObject<IVsDataProvider>, IBsDataConnectionProperties,
	...
	IEnumerable, IVsDataConnectionUIProperties, ICustomTypeDescriptor, INotifyPropertyChanged
{
	...
	public virtual void Parse(string connectionString)
	{
		try
		{
			...
			lock (_LockObject)
			{
				...
				// Evs.Debug(GetType(), nameof(Parse), "ConnectionSource: {0}, connectionString: {1}", ConnectionSource, ConnectionStringBuilder.ConnectionString);
			}
	...
	{
		// Evs.Trace(GetType(), nameof(ContainsKey), "key: {0}", key);
