...
internal class ErmBindingSource : BindingSource, System.Collections.IEnumerable
{
	...
	internal event EventHandler DependencyCurrentChanged
	{
		add
		{
			...
			Events.AddHandler(EVENT_DEPENDENCYCURRENTCHANGED, value);
		}
		remove
		{
			if (Dependent == null)
			...
			Events.RemoveHandler(EVENT_DEPENDENCYCURRENTCHANGED, value);
		}
	}
	...
	internal event ListChangedEventHandler DependencyListChanged
	{
		add
		{
			Events.AddHandler(EVENT_DEPENDENCYLISTCHANGED, value);
		}
		remove
		{
			Events.RemoveHandler(EVENT_DEPENDENCYLISTCHANGED, value);
		}
	}
	...
	internal event EventHandler DependencyPositionChanged
	{
		add
		{
			...
			Events.AddHandler(EVENT_DEPENDENCYPOSITIONCHANGED, value);
		}
		remove
		{
			if (Dependent == null)
			...
			Events.RemoveHandler(EVENT_DEPENDENCYPOSITIONCHANGED, value);
		}
	}
	...
	// ---------------------------------------------------------------------------------
	/// <summary>
	/// The column in the <see cref="DependentSource"/> DataSource that will serve as
	...
	// ---------------------------------------------------------------------------------
	/// <summary>
	/// True if <see cref="DataSource"/>, <see cref="DependentSource"/>,
	/// <see cref="PrimaryKey"/> and <see cref="ForeignKey"/> have been set.
	/// </summary>
	/// <remarks>
	...
	// ---------------------------------------------------------------------------------
	/// <summary>
	/// The null value of the <see cref="ForeignKey"/> if
	/// <see cref="BindingSource.Current"/> is invalidated. The default 'null' assumes
	/// the ForeignKey is nullable else you can use an empty string or any other value
	...
	// ---------------------------------------------------------------------------------
	/// <summary>
	/// The column in the master <see cref="BindingSource.DataSource"/> that will serve
	...
	// ---------------------------------------------------------------------------------
	/// <summary>
	/// Gets the <see cref="BindingSource.Current"/> object as a DataRow else returns
	/// null.
	/// </summary>
	///
	// ---------------------------------------------------------------------------------
	...
	// ---------------------------------------------------------------------------------
	/// <summary>
	/// Sets the master <see cref="BindingSource"/>'s cursor
	/// <see cref="BindingSource.Position"/> to zero and updates the
	/// <see cref="Dependent"/>'s filter once <see cref="IsReady"/> is true.
	/// </summary>
	// ---------------------------------------------------------------------------------
	...
	protected void UpdateDependencyFilter(bool initializing = false)
	{
		...
		try
		{
			...
			// Evs.Trace("Dependency filter set: Position: " + DependentPosition);
