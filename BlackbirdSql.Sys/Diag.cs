...
public static class Diag
{
...
#endif
	{
		...
		if (DbNative.DatabaseEngineSvc != null && ex is DbException exf && DbNative.DbExceptionSvc.HasSqlException(exf))
		{
			message += Environment.NewLine + $"{DbNative.DbEngineName} error code: {DbNative.DbExceptionSvc.GetErrorCode(exf)}, Class: {DbNative.DbExceptionSvc.GetClass(exf)}, Proc: {DbNative.DbExceptionSvc.GetProcedure(exf)}, Line: {DbNative.DbExceptionSvc.GetLineNumber(exf)}.";
		}
