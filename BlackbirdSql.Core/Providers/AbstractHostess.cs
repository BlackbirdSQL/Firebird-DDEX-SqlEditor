...
public abstract class AbstractHostess : IDisposable
{
	...
	public bool IsCommandUIContextActive(Guid commandUIGuid)
	{
		...
		if (service == null)
		{
			InvalidOperationException ex = new("IVsMonitorSelection service not found");
			Diag.Dug(ex);
			throw ex;
		}
		...
		try
		{
			Native.WrapComCall(service.GetCmdUIContextCookie(ref commandUIGuid, out uint pdwCmdUICookie));
			Native.WrapComCall(service.IsCmdUIContextActive(pdwCmdUICookie, out pfActive));
		}
		catch (Exception ex)
		{
			Diag.Dug(ex);
