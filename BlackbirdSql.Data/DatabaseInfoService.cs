...
public class DatabaseInfoService : SBsNativeDatabaseInfo, IBsNativeDatabaseInfo
{
	...
	{
		return (long)Reflect.InvokeGenericMethod<long>(@this.NativeObject, "GetValue", BindingFlags.Default, [(byte)DbIscCodes.isc_info_delete_count]);
	}
	...
	public List<(long, long)> GetTablesDatabaseInfo(NativeDatabaseInfoProxy @this)
	{
		...
		byte[] iscValues =
			[
				Convert.ToByte(DbIscCodes.isc_info_insert_count), // 25
				Convert.ToByte(DbIscCodes.isc_info_update_count), // 26
				Convert.ToByte(DbIscCodes.isc_info_delete_count), // 27
				Convert.ToByte(DbIscCodes.isc_info_read_idx_count), // 24
				Convert.ToByte(DbIscCodes.isc_info_read_seq_count), // 23
				Convert.ToByte(DbIscCodes.isc_info_expunge_count), // 30
				Convert.ToByte(DbIscCodes.isc_info_purge_count) // 29
			];
		...
		{
			byte[] requestBuffer = [brequest, Convert.ToByte(DbIscCodes.isc_info_end)];
			byte[] responseBuffer = new byte[DbIscCodes.DEFAULT_MAX_BUFFER_SIZE];
	...
	{
		return (long)Reflect.InvokeGenericMethod<long>(@this.NativeObject, "GetValue", BindingFlags.Default, [(byte)DbIscCodes.isc_info_purge_count]);
	}
