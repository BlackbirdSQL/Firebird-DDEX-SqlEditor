...
public class EfbDefaultMigrationSqlGeneratorBehavior : IEfbMigrationSqlGeneratorBehavior
{
	public virtual IEnumerable<string> CreateIdentityForColumn(string columnName, string tableName)
	{
		...
		using (var writer = EfbMigrationSqlGenerator.SqlWriter())
		{
		...
		using (var writer = EfbMigrationSqlGenerator.SqlWriter())
		{
			...
			writer.Write(EfbMigrationSqlGenerator.Quote(CreateTriggerName(columnName, tableName)));
			writer.Write(" ACTIVE BEFORE INSERT ON ");
			writer.Write(EfbMigrationSqlGenerator.Quote(tableName));
			writer.WriteLine();
			writer.WriteLine("AS");
			writer.WriteLine("BEGIN");
			writer.Indent++;
			writer.Write("if (new.");
			writer.Write(EfbMigrationSqlGenerator.Quote(columnName));
	...
	public virtual IEnumerable<string> DropIdentityForColumn(string columnName, string tableName)
	{
		...
		using (var writer = EfbMigrationSqlGenerator.SqlWriter())
		{
			...
			writer.Write(EfbMigrationSqlGenerator.Quote(triggerName));
			writer.Write("';");
			writer.WriteLine();
			writer.Indent--;
			writer.WriteLine("end");
