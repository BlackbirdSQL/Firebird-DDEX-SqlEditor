...
#region Static Methods
	...
	internal static string GenerateUpdateSql(DbUpdateCommandTree tree, out List<DbParameter> parameters, bool generateParameters = true)
	{
		...
		foreach (DbSetClause setClause in tree.SetClauses.Cast<DbSetClause>())
		{
			if (first)
			{
				first = false;
		...
		DbExpression returning)
	{
		...
		var columnsToFetch =
			table.ElementType.Members
				...
				.Except(tree is not DbInsertCommandTree ? table.ElementType.KeyMembers : Enumerable.Empty<EdmMember>());
		...
		if (translator.Parameters.Any())
		{
			...
			foreach (DslParameter param in translator.Parameters.Cast<DslParameter>())
			{
				...
				if (param.FbDbType == DslDbType.VarChar || param.FbDbType == DslDbType.Char)
