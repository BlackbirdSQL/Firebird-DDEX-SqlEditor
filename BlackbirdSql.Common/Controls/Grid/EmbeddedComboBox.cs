	...
	public class EmbeddedComboBox : ComboBox, IBGridEmbeddedControl, IBGridEmbeddedControlManagement2, IBGridEmbeddedControlManagement
	{
		...
		public int SetCurSelectionAsString(string strNewSel)
		{
			...
			if (num == -1)
			{
				...
				{
					InvalidOperationException ex = new(ControlsResources.InvalidSelStringInEmbeddedCombo);
					Diag.Dug(ex);
					throw ex;
				}
		...
		public void SetCurSelectionIndex(int nIndex)
		{
			...
			{
				ArgumentException ex = new(ControlsResources.InvalidSelIndexInEmbeddedCombo, "nIndex");
				Diag.Dug(ex);
				throw ex;
			}
