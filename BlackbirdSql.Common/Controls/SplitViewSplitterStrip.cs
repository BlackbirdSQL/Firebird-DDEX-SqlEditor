...
public class SplitViewSplitterStrip : ToolStrip
{
	private class SplitBarRenderer : BarBackgroundRenderer
	{
		...
		private void RenderHorizontalBackgroundTabs(ToolStripRenderEventArgs e)
		{
			...
			if (!splitViewSplitterStrip.ShowSplitter)
			{
				...
				using (Pen pen = new Pen(VsColorUtilities.GetShellColor(-216)))
				{
			...
			else
			{
				...
				using (Pen pen2 = new Pen(VsColorUtilities.GetShellColor(-216)))
				{
				...
				using (Pen pen3 = new Pen(VsColorUtilities.GetShellColor(-216)))
				{
		...
		private void RenderVerticalBackgroundTabs(ToolStripRenderEventArgs e)
		{
			...
			if (!splitViewSplitterStrip.ShowSplitter)
			{
				...
				using (Pen pen = new Pen(VsColorUtilities.GetShellColor(-212)))
				{
			...
			else
			{
				...
				using (Pen pen2 = new Pen(VsColorUtilities.GetShellColor(-212)))
				{
				...
				using (Pen pen3 = new Pen(VsColorUtilities.GetShellColor(-212)))
				{
		...
		private void RenderDownTab(Graphics g, Rectangle tabBounds, bool isActive)
		{
			...
			if (!isActive)
			{
				using Brush brush = new LinearGradientBrush(tabBounds, base.VSCOLOR_FILETAB_GRADIENTDARK, base.VSCOLOR_FILETAB_GRADIENTHIGHLIGHT, LinearGradientMode.Vertical);
				g.FillPolygon(brush, points);
			}
			else
			{
				using Brush brush2 = new SolidBrush(base.VSCOLOR_FILETAB_SELECTEDBACKGROUND);
				g.FillPolygon(brush2, points);
			}
			g.DrawLine(SystemPens.ControlDark, point, point2);
			g.DrawLine(SystemPens.ControlDark, point3, point4);
			using (Pen pen = new Pen(base.ColorTable.ImageMarginRevealedGradientEnd))
			{
				g.DrawLine(pen, point.X - 1, point.Y, point2.X - 1, point2.Y);
			}
			using (Pen pen2 = new Pen(base.VSCOLOR_FILETAB_GRADIENTHIGHLIGHT))
			{
		...
		private void RenderLeftTab(Graphics g, Rectangle tabBounds, bool isActive)
		{
			...
			if (!isActive)
			{
				using Brush brush = new LinearGradientBrush(tabBounds, base.VSCOLOR_FILETAB_GRADIENTHIGHLIGHT, base.VSCOLOR_FILETAB_GRADIENTDARK, LinearGradientMode.Horizontal);
				g.FillPolygon(brush, points);
			}
			else
			{
			...
			using (Pen pen = new Pen(base.VSCOLOR_FILETAB_GRADIENTHIGHLIGHT))
			{
				g.DrawLine(pen, point.X - 1, point.Y + 1, point2.X - 1, point2.Y + 1);
			}
			using (Pen pen2 = new Pen(base.ColorTable.ImageMarginRevealedGradientEnd))
			{
				g.DrawLine(SystemPens.ControlDark, point5.X - 1, point5.Y + 1, point6.X, point6.Y);
				g.DrawLine(pen2, point5.X - 1, point5.Y, point6.X - 1, point6.Y);
		...
		private void RenderRightTab(Graphics g, Rectangle tabBounds, bool isActive)
		{
			...
			if (!isActive)
			{
				using Brush brush = new LinearGradientBrush(tabBounds, base.VSCOLOR_FILETAB_GRADIENTDARK, base.VSCOLOR_FILETAB_GRADIENTHIGHLIGHT, LinearGradientMode.Horizontal);
				g.FillPolygon(brush, points);
			}
			else
			{
		...
		private void RenderUpTab(Graphics g, Rectangle tabBounds, bool isActive)
		{
			...
			if (!isActive)
			{
				using Brush brush = new LinearGradientBrush(tabBounds, base.VSCOLOR_FILETAB_GRADIENTHIGHLIGHT, base.VSCOLOR_FILETAB_GRADIENTDARK, LinearGradientMode.Vertical);
				g.FillPolygon(brush, points);
			}
			else
			{
		...
		protected override void OnRenderItemText(ToolStripItemTextRenderEventArgs e)
		{
			...
			{
				e.TextColor = VsColorUtilities.GetShellColor(-219);
		...
		protected override void OnRenderItemImage(ToolStripItemImageRenderEventArgs e)
		{
			...
			if (e.Image != null)
			{
				...
				array[0].NewColor = ((flag3 && SystemInformation.HighContrast) ? VsColorUtilities.GetShellColor(-203) : VsColorUtilities.GetShellColor(-219));
	...
	public ToolStripButton HSplitButton
	{
		get
		{
			if (_hSplitButton == null)
			{
				...
				_hSplitButton.Checked = _showSplitter && base.Orientation == Orientation.Horizontal;
	...
	public ToolStripButton VSplitButton
	{
		get
		{
			if (_vSplitButton == null)
			{
				...
				_vSplitButton.Checked = _showSplitter && base.Orientation == Orientation.Vertical;
	...
	public bool ShowSplitter
	{
		...
		set
		{
			...
			_showSplitter = value;
			HSplitButton.Checked = _showSplitter && base.Orientation == Orientation.Horizontal;
	...
	public SplitViewSplitterStrip(IServiceProvider serviceProvider)
	{
		...
		_lastOrientation = base.Orientation;
		...
		base.Renderer = new SplitBarRenderer(serviceProvider);
	...
	protected override object GetService(Type service)
	{
		if (base.Parent != null)
		{
			object service2 = ((IServiceProvider)base.Parent).GetService(service);
			if (service2 != null)
			{
				return service2;
			}
	...
	protected override void OnLayout(LayoutEventArgs e)
	{
		if (_primaryPaneButtons != null && _secondaryPaneButtons != null)
		{
			...
			if (!ShowSplitter)
			{
				...
				{
					PrimaryPaneButtons[i].Margin = ((base.Orientation == Orientation.Horizontal) ? new Padding(num3 + S_FINASCENT, 0, 0, 0) : new Padding(0, num3 + S_FINASCENT, 0, 0));
				}
				for (int j = 0; j < SecondaryPaneButtons.Count - 1; j++)
				{
					SecondaryPaneButtons[j].Margin = ((base.Orientation == Orientation.Horizontal) ? new Padding(num3 + S_FINASCENT, 0, 0, 0) : new Padding(0, num3 + S_FINASCENT, 0, 0));
				}
				SecondaryPaneLastVisibleOrDefault.Margin = ((base.Orientation == Orientation.Horizontal) ? new Padding(num3 + S_FINASCENT, 0, SecondaryPaneLastVisibleOrDefault.Height, 0) : new Padding(0, num3 + S_FINASCENT, 0, SecondaryPaneLastVisibleOrDefault.Width));
			}
			else
			{
				for (int k = 0; k < PrimaryPaneButtons.Count - 1; k++)
				{
					PrimaryPaneButtons[k].Margin = ((base.Orientation == Orientation.Horizontal) ? new Padding(0, 0, num3 + S_FINASCENT, 0) : new Padding(0, 0, 0, num3 + S_FINASCENT));
				}
				PrimaryPaneLastVisibleButtonOrDefault.Margin = ((base.Orientation == Orientation.Horizontal) ? new Padding(0, 0, PrimaryPaneLastVisibleButtonOrDefault.Height, 0) : new Padding(0, 0, 0, PrimaryPaneLastVisibleButtonOrDefault.Width));
				for (int l = 0; l < SecondaryPaneButtons.Count - 1; l++)
				{
					SecondaryPaneButtons[l].Margin = ((base.Orientation == Orientation.Horizontal) ? new Padding(0, 0, num3 + S_FINASCENT, 0) : new Padding(0, 0, 0, num3 + S_FINASCENT));
				}
				SecondaryPaneLastVisibleOrDefault.Margin = ((base.Orientation == Orientation.Horizontal) ? new Padding(0, 0, SecondaryPaneLastVisibleOrDefault.Height, 0) : new Padding(0, 0, 0, SecondaryPaneLastVisibleOrDefault.Width));
			}
		}
		else
		{
		...
		if (ShowSplitter)
		{
			if (PrimaryPaneButtons != null)
			{
				foreach (ToolStripButton primaryPaneButton in PrimaryPaneButtons)
				{
					...
					{
						Point location = ((base.Orientation == Orientation.Horizontal) ? new Point(primaryPaneButton.Bounds.X, primaryPaneButton.Bounds.Y - 2) : new Point(primaryPaneButton.Bounds.X - 2, primaryPaneButton.Bounds.Y));
			...
			}
			{
				foreach (ToolStripButton secondaryPaneButton in SecondaryPaneButtons)
				{
					...
					{
						Point location2 = ((base.Orientation == Orientation.Horizontal) ? new Point(secondaryPaneButton.Bounds.X, secondaryPaneButton.Bounds.Y + 2) : new Point(secondaryPaneButton.Bounds.X + 2, secondaryPaneButton.Bounds.Y));
		...
		if (PrimaryPaneButtons != null)
		{
			foreach (ToolStripButton primaryPaneButton2 in PrimaryPaneButtons)
			{
				...
				{
					Point location3 = ((base.Orientation == Orientation.Horizontal) ? new Point(primaryPaneButton2.Bounds.X, primaryPaneButton2.Bounds.Y - 2) : new Point(primaryPaneButton2.Bounds.X - 2, primaryPaneButton2.Bounds.Y));
		...
		foreach (ToolStripButton secondaryPaneButton2 in SecondaryPaneButtons)
		{
			Point location4 = ((base.Orientation == Orientation.Horizontal) ? new Point(secondaryPaneButton2.Bounds.X, secondaryPaneButton2.Bounds.Y - 2) : new Point(secondaryPaneButton2.Bounds.X - 2, secondaryPaneButton2.Bounds.Y));
	...
	protected override void OnLayoutStyleChanged(EventArgs e)
	{
		...
		if (_lastOrientation != base.Orientation)
		{
			_lastOrientation = base.Orientation;
			UpdateMinimumSize();
			UpdateChevronButton();
			SizeF scaleFactor = ControlUtils.GetScaleFactor(this);
			SwapButton.Image = ControlUtils.ScaleImage((base.Orientation == Orientation.Horizontal) ? ControlsResources.swap_horz : ControlsResources.swap_vert, scaleFactor);
		}
		UpdateOrientation();
	}
	...
	protected override void OnLayoutCompleted(EventArgs e)
	{
		...
		if (toolStripItem2 != null && toolStripItem != null)
		{
			if (base.Orientation == Orientation.Horizontal)
			{
				...
				_splitterRectangle = Rectangle.FromLTRB(toolStripItem2.Bounds.Right + 1, 0, toolStripItem.Bounds.Left - 1, base.Height);
			}
			else
			{
				_splitterRectangle = Rectangle.FromLTRB(0, toolStripItem2.Bounds.Bottom + 1, base.Width, toolStripItem.Bounds.Top);
			}
		}
		else
		{
	...
	protected override void OnMouseMove(MouseEventArgs mea)
	{
		...
		{
			Cursor.Current = ((base.Orientation == Orientation.Vertical) ? Cursors.VSplit : Cursors.HSplit);
		}
		else if (Cursor.Current != Cursors.Default && !base.Capture)
		{
	...
	private void UpdateChevronButton()
	{
		...
		if (ShowSplitter)
		{
			ChevronButton.Image = ControlUtils.ScaleImage((base.Orientation == Orientation.Horizontal) ? ControlsResources.horizontal_collapse : ControlsResources.vertical_collapse, scaleFactor);
			ChevronButton.Text = "-";
		}
		else
		{
			ChevronButton.Image = ControlUtils.ScaleImage((base.Orientation == Orientation.Horizontal) ? ControlsResources.horizontal_expand : ControlsResources.vertical_expand, scaleFactor);
			ChevronButton.Text = "+";
		}
	}
	...
	private void UpdateMinimumSize()
	{
		...
		_minDistance = ((base.Orientation == Orientation.Horizontal) ? base.PreferredSize.Width : base.PreferredSize.Height);
	...
	private void UpdateOrientation()
	{
		...
		ToolStripItemDisplayStyle displayStyle = ((base.Orientation == Orientation.Horizontal) ? ToolStripItemDisplayStyle.ImageAndText : ToolStripItemDisplayStyle.Image);
