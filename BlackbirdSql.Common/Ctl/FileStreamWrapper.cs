...
public class FileStreamWrapper : IFileStreamWrapper, IDisposable
{
	...
	public void Init(string sFileName, bool bOpenExisting, int iBufferSize)
	{
		if (m_bInitialized)
		{
			throw new Exception(SharedResx.FileStreamAlreadyInitialized);
		}
		if (iBufferSize <= 0)
		{
			throw new Exception(SharedResx.BufferSizeShouldBePositive);
		}
	...
	public int ReadData(byte[] buf, int iBytes, long i64Offset)
	{
		if (!m_bInitialized)
		{
			throw new Exception(SharedResx.FileStreamNeedsToBeInitializedFirst);
		}
		MoveTo(i64Offset);
		int num = 0;
		while (num < iBytes)
